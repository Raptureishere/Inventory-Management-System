...
const LowStockTable: React.FC<{ items: Item[] }> = ({ items }) => (
  <div className="bg-white p-6 rounded-xl shadow-sm hover:shadow-lg transition-shadow duration-300 h-full">
    <h3 className="text-lg font-semibold text-slate-800 hover:text-amber-600 transition-colors duration-300 mb-4 flex items-center cursor-default">
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm text-left text-slate-500">
        ...
        <tbody>
          {items.map(item => (
            <tr key={item.id} className="bg-white border-b border-slate-200 hover:bg-slate-50 transition-colors duration-200 cursor-pointer">
...
const RecentRequisitionsTable: React.FC<{ requisitions: Requisition[] }> = ({ requisitions }) => (
  <div className="bg-white p-6 rounded-xl shadow-sm hover:shadow-lg transition-shadow duration-300 h-full">
    <h3 className="text-lg font-semibold text-slate-800 hover:text-sky-600 transition-colors duration-300 mb-4 cursor-default">Recent Requisitions</h3>
    <div className="overflow-x-auto">
       <table className="w-full text-sm text-left text-slate-500">
        <thead className="text-xs text-slate-700 uppercase bg-slate-50">
          <tr>
        ...
        <tbody>
          {requisitions.map(req => (
            <tr key={req.id} className="bg-white border-b border-slate-200 hover:bg-slate-50 transition-colors duration-200 cursor-pointer">
...
const InventoryPieChart: React.FC<{ items: Item[] }> = ({ items }) => {
    ...
    const largestCategory = data.reduce((max, item) => item.value > max.value ? item : max, data[0] || { name: '', value: 0 });
    const largestCategoryPercentage = totalItems > 0 ? ((largestCategory.value / totalItems) * 100).toFixed(1) : '0';
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm hover:shadow-lg transition-shadow duration-300">
            <h3 className="text-lg font-semibold text-slate-800 hover:text-emerald-600 transition-colors duration-300 mb-4 cursor-default">Stock by Category</h3>
...
const TopIssuedItemsChart: React.FC<{ items: Item[], issuedRecords: IssuedItemRecord[] }> = ({ items, issuedRecords }) => {
    ...
    if (data.length === 0) {
        return (
            <div className="bg-white p-6 rounded-xl shadow-sm hover:shadow-lg transition-shadow duration-300 flex flex-col justify-center items-center h-full">
                <h3 className="text-lg font-semibold text-slate-800 hover:text-purple-600 transition-colors duration-300 mb-4 cursor-default">Top 5 Issued Items</h3>
