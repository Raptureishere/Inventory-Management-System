...
const Reports: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setIsLoading(true);
                setError(null);
                const [itemsData, issuedRecordsData] = await Promise.all([
    ...
    const itemMovementData = useMemo(() => {
        // This logic should ideally be handled by a dedicated backend endpoint for efficiency.
        return items.map(item => ({
            name: item.itemName,
            stockIn: 0, // Stock-in data is not available in this system
            stockOut: issuedRecords
                .flatMap(rec => rec.issuedItems)
                .filter(iss => iss.itemId === item.id)
                .reduce((sum, iss) => sum + iss.issuedQty, 0),
        })).filter(d => d.stockOut > 0).slice(0, 10);
    }, [items, issuedRecords]);

    const stockQuantityByCategoryData = useMemo(() => {
        // Backend should handle date filtering for accuracy.
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select value={reportType} onChange={e => setReportType(e.target.value)} className="mt-1 block w-full p-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            ...
                            <option value="item_movement">Item Movement (Stock Out)</option>
